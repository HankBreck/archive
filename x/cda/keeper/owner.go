...
func (k Keeper) AppendOwnerCDA(ctx sdk.Context, owner string, cdaId uint64) error {
	...
	count := k.GetOwnerCDACount(ctx, ownerAddr)
...
func (k Keeper) GetOwnerCDACount(ctx sdk.Context, owner sdk.AccAddress) uint64 {
	...
	byteCount := store.Get(owner.Bytes())
...
func (k Keeper) SetOwnerCDACount(ctx sdk.Context, owner sdk.AccAddress, count uint64) {
	...
	store.Set(owner.Bytes(), byteCount)
}
